...
基于 Vite、TypeScript 与 Three.js 的双人同盘自走棋。2 名真人各自运营商店与经济，共享棋盘羁绊和 100 生命，与 7 支机器人队竞争。首版实现 20 枚棋子、8 个羁绊、6 件装备、野怪、淘汰排名和完整回合循环。
...
- 中央金色格用于交付。队友选中公共棋子，再点自己的空备战格领取；交付者可用同样方式撤回。公共区不自动合成。
- 拖动自己的非公共区棋子时会出现出售区。也可在选中浮层中出售；装备返回背包。棋盘上的队友棋子只能调整站位。
...
目前所有模型均为程序生成的低多边形棋子，未使用参考图中的原作模型或贴图。部分棋子标签和技能为了首版 8 羁绊池进行了改编。50 棋子、异虫特殊合成、建筑和肉鸽模式属于后续阶段。
